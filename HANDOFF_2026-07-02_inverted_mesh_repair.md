...
## 5-γ. 07-05: CUDA 재스윕 완료 + 타이밍 표 보류

- **CUDA 전 그리드 재스윕 완료** (커밋 408a7f0). 대형 슬로다운 해결(E8 27,646s→723s, 38×),
  결정성 회복. E3/E6의 0-셀은 CPU와 일치하는 **진짜 자기지지 방향**(CUDA 버그 아님) —
  E3 pending 자동 해제.
- **★ 성능 회귀 발견 → 타이밍 표 보류**: 07-04 정확성 수정 dll이 CPU/CUDA 모두 작은~중간
  메시에서 ~4배(CUDA 소형 ~15배) 느려짐 (cube CUDA 1° 11s→168s, CPU 그룹A 43s→167s).
  방향당 비용 증가(고정 오버헤드 아님). 원인: CUDA 슬롯 용량 64→512, 절단 패스 추가;
  CPU 별도 int32 배열 재저장. → **dll팀에 최적화 요청함**:
  `Tomo_Shell2026-dev/PERF_REGRESSION_2026-07-05_correctness_fix.md`.
- **보류 중**: S1·표 10의 타이밍 열, 초록·기여의 속도향상 범위. 최적화 dll로 재측정 후 확정.
  (참고: 현 수정 dll 기준이면 py 7.2–1703×, C++ 101–249,672×로 오히려 커지나, 느려진
  baseline 탓이라 미최적화 상태로 확정하지 않기로 함 — 저자 결정 B.)
- 그 외 정확성 기반 정정(표 2–5, budget-curve, LOMO, unified, 보충 S3/S5/S6, 한계 서브섹션,
  컨투어 그림, 초록/기여/결론 스코프 축소)은 **전부 완료·push**됨.

## 5-δ. 07-05: 전부 완료

- **07-05 성능 최적화 dll 배포·검증** (커밋 9c613df): CPU 완전 회복(cube 1° 93s→13s),
  CUDA 개선(E8 723s→592s), 정확성 유지(음수 0, 0-셀은 CPU와 일치하는 자기지지, best 안정,
  그리드 값 불변). 성능 회귀 해결.
- **최적화 dll 단일 세션으로 35메쉬 1°/60° 전체 재측정** (커밋 0abd3b6). 최종 확정:
  - 속도향상 Python 4.7–102.3×, C++ 86–16,075× (정상 baseline 기준 정직한 값).
  - CUDA는 CPU보다 평균 3.1× 느림 → 성능향상은 알고리즘적임이 더 명확.
  - S1(35행)·표 10·기여·CUDA 프로즈 갱신, 캡션 단일세션으로 정정. 두 문서 컴파일 OK.
- **→ 논문 revision의 모든 TOMO 의존 숫자가 정정·최적화 dll 기준으로 통일됨. 전부 완료.**
  남은 것은 사용자 액션(편집자 통지, docx 최종본 생성)뿐.
