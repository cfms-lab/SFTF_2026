...
def main():
    ...
    for i, r in enumerate(rows, start=1):
        ...
        t0 = time.perf_counter(); vals = [evaluate_sftf_v2(mesh, d, config=cfg8, samples=surf) for d in directions]; t_score = time.perf_counter() - t0
        ...
        t0 = time.perf_counter(); surf4 = deterministic_surface_samples(mesh, 4096)
        vals4 = [evaluate_sftf_v2(mesh, d, config=cfg4, samples=surf4) for d in directions]; t_gate = time.perf_counter() - t0
        rec = {"holdout_id": r["holdout_id"], "stratum": r["stratum"], "face_count": int(r["face_count"]),
               ...
               "t_uniform_cells_s": t_uniform, "t_gate_4096_sample_and_score_s": t_gate,
               "t_gen_ungated_s": t_sample + t_score + t_nms + t_cells,
               "t_gen_gated_s": t_sample + t_score + t_nms + t_cells + t_gate,
               "n_cells_2400": int(len(cells2400))}
        records.append(rec)
        print(f"[{i}/{len(rows)}] {r['holdout_id']} faces={r['face_count']} sample={t_sample:.2f} score={t_score:.2f} nms={t_nms:.3f} cells={t_cells:.2f} gate={t_gate:.2f} -> T_gen={rec['t_gen_ungated_s']:.2f}s", flush=True)
    def agg(key):
