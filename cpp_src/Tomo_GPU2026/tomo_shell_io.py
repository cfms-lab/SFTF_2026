"""Tomo_Shell2026 Python 유틸리티.

ctypes 포인터 변환, 메시 로딩, 회전 행렬, 결과 시각화, 최적 방향 탐색처럼
main.py와 tomo_shell_cpp.py가 공유하는 보조 기능을 모아 둔 모듈이다.
"""
...
# Windows DLL은 공유 배포된 Tomo_Shell2026.dll을 사용한다(_shared_dll에서 복사).
# solid TOMO_CPU 경로는 TomoSh_INT3/CUDA를 shell_mesh=0으로 호출한다(tomo_shell_cpp.py 참고).
g_Cdll = None
g_CppDLLFileName = str(Path(__file__).resolve().parent / 'Tomo_Shell2026.dll')
...
g_PixelColors    = (   'green',  'purple',     'red',   'orange',     'red',    'blue',   'gray',    'red',     'red', 'sandybrown',    'aqua') #https://community.plotly.com/t/plotly-colours-list/11730/2
...
#--------------------------------------------------------------
#com. geom.
def getBoundary(vtx): # NumPy [x,y,z] 좌표 배열의 AABB를 계산한다.
...
def Plot3DPixels( self, title=""):# Plotly로 Al/Be/SS/Bed 등 픽셀 그룹을 3D 표시한다.
...
def LoadInputMesh(filename):
  ...
  # Lightweight OBJ parser (no external dependency)
  if ext == '.obj':
    with open(filename, 'r') as f:
      for line in f:
        if line.startswith('v '):
          parts = line.split()
          if len(parts) >= 4:
            vtx.append([float(parts[1]), float(parts[2]), float(parts[3])])
    ...
    # Try using trimesh as a fallback for other formats
    try:
      import trimesh
      mesh_tm = trimesh.load(filename, process=False)
      vtx = mesh_tm.vertices.tolist()
      if hasattr(mesh_tm, 'faces'):
        tri = mesh_tm.faces.tolist()
      else:
        print(Fore.RED + '[ERROR] Loaded mesh has no faces: ' + filename + Style.RESET_ALL)
        sys.exit(0)
    except Exception as e:
      print(Fore.RED + '[ERROR] Unsupported mesh format or trimesh missing: ' + str(e) + Style.RESET_ALL)
      sys.exit(0)

  vtx = np.array(vtx, dtype=np.float32)
  if vtx.size == 0:
    print(Fore.RED + '[ERROR] No vertices read from: ' + filename +  Style.RESET_ALL)
    sys.exit(0)
  tri = np.array(tri, dtype=np.int32) if len(tri) > 0 else np.zeros((0,3), dtype=np.int32)

  # An inside-out mesh (negative signed volume) flips all winding-derived
  # normals below, inverting overhang detection; rewind to consistent outward
  # faces (also fixes mixed-winding files like pipe_elbow).
  if tri.size > 0:
    from .mesh_orientation import orient_faces_outward, signed_volume
    volume0 = signed_volume(vtx, tri)
    if volume0 < 0.0:
      print(Fore.YELLOW + '[WARN] %s: negative signed volume (%.6g); mesh is inside-out -- '
            'rewinding faces to consistent outward orientation.'
            % (filename, volume0) + Style.RESET_ALL)
      tri = np.ascontiguousarray(orient_faces_outward(vtx, tri))

  # triangle normals
  if tri.size == 0:
    tri_nrm = np.zeros((0,3), dtype=np.float32)
  else:
    u = vtx[tri[:,1]] - vtx[tri[:,0]]
    v = vtx[tri[:,2]] - vtx[tri[:,0]]
    n = np.cross(u, v)
    norms = np.linalg.norm(n, axis=1)
    nz = norms > 1e-8
    n[nz] = (n[nz].T / norms[nz]).T
    tri_nrm = n.astype(np.float32)

  # vertex normals (area-weighted)
  vtx_nrm = np.zeros_like(vtx, dtype=np.float32)
  if tri.size > 0:
    tri_area = getMeshTriArea(vtx, tri)
    for i in range(tri.shape[0]):
      for j in range(3):
        vtx_nrm[ tri[i,j] ] += tri_nrm[i] * tri_area[i]
    vnorms = np.linalg.norm(vtx_nrm, axis=1)
    nzv = vnorms > 1e-8
    vtx_nrm[nzv] = (vtx_nrm[nzv].T / vnorms[nzv]).T
  else:
    tri_area = np.zeros((0,), dtype=np.float32)

  tr_center = np.average( vtx[tri], axis=1).astype(np.float32) if tri.size>0 else np.zeros((0,3), dtype=np.float32)

  (g_mesh0_min, g_mesh0_max, g_mesh0_center) = getBoundary( vtx)
  # translate to center
  vtx = vtx - g_mesh0_center

  global g_input_mesh_filename, g_mesh0_surface_area
  g_input_mesh_filename = filename
  g_mesh0_surface_area = float(np.sum(tri_area))

  # Convex hull
  try:
    hull = ConvexHull(vtx)
    chull_tri = np.asarray(hull.simplices, dtype=np.int32)
    chull_vtx = np.asarray(vtx[hull.vertices], dtype=np.float32)
    if chull_tri.size > 0:
      u = chull_vtx[chull_tri[:,1]] - chull_vtx[chull_tri[:,0]]
      v = chull_vtx[chull_tri[:,2]] - chull_vtx[chull_tri[:,0]]
      n = np.cross(u, v)
      norms = np.linalg.norm(n, axis=1)
      nz = norms > 1e-8
      n[nz] = (n[nz].T / norms[nz]).T
      chull_trinrm = n.astype(np.float32)
    else:
      chull_trinrm = np.zeros((0,3), dtype=np.float32)
  except Exception:
    chull_tri = np.zeros((0,3), dtype=np.int32)
    chull_vtx = np.zeros((0,3), dtype=np.float32)
    chull_trinrm = np.zeros((0,3), dtype=np.float32)

  # Lightweight mesh-like container used elsewhere in the code
  class SimpleMesh:
    def __init__(self, vertices, triangles, vertex_normals, triangle_normals):
    ...
    def simplify_vertex_clustering(self, voxel_size=None, contraction=None):
      # No-op fallback for simplification (keeps original mesh)
      return self
    def get_surface_area(self):
        if self.triangles.size == 0:
          return 0.0
        u = self.vertices[self.triangles[:,1]] - self.vertices[self.triangles[:,0]]
        v = self.vertices[self.triangles[:,2]] - self.vertices[self.triangles[:,0]]
        n = np.cross(u, v)
        area = 0.5 * np.linalg.norm(n, axis=1)
        return float(np.sum(area))

  mesh0 = SimpleMesh(vtx, tri, vtx_nrm, tri_nrm)
