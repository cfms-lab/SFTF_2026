...
## 2차 결과의 해석 한정 (2026-09-17 내부 검토 반영)

- 30메쉬 패널은 최초 선정 시 봉인됐던 표본이지만, uniform·SFTF 단독 결과는 이미 계산·공개
  (Table 3)돼 있었다. 동일 패널 정규화에서 NRR(혼합) = min(NRR(U5), NRR(S5))이므로 2부는
  **기존 관측의 새 결합에 대한 사전 지정 재분석**이지 미열람 전향 확증이 아니다.
  원고·답변서에서는 "confirmed" 대신 "prespecified criterion met in a reanalysis of previously
  analyzed data" 로 표기하고, `sealed` 는 최초 선정 상태에만 쓴다.
- 결측: Cura 크래시 P012·P034는 uniform-5 셀이라 70메쉬 혼합 분석은 그 두 메쉬에서 9/10 방향
  best-available 이다. 사후 complete-case 민감도(n=68, seed 30): −0.0387 [−0.0591, −0.0186],
  p=0.00027 (`hybrid_sensitivity.py`, 사전 지정 아님).
- 혼합이 uniform보다 나쁜 메쉬: 70개에서 22(Cura)/20(Prusa), 30개에서 7/9. 최대 손실은
  SFTF 단독 0.44/0.64 → 혼합 0.13/0.14. "removed the tail" 이 아니라 "reduced the adverse tail".
- 일반화: 검정된 것은 B=10의 5+5 뿐. B=20의 10+10은 이득 없음. 50:50 이외 비율은 미평가.
- 감사 추적: 두 잠금의 GATE 해시는 이 파일의 동결 당시 판본이며, `GATE_snapshots/` 에 바이트
  동일 복원본(해시 일치 확인)을 둔다. 잠금 시각은 로컬 시계이며 외부 등록은 없다.
  2부 분석 로직은 `analyze_part2.py` 로 저장(원자료에서 오차 0 재현).
- T_gen: 투고본의 4.57 s 는 K=8,192 점수 루프만의 시간이다. 샘플링·NMS·셀 확장을 포함한
  전체 생성 경로를 `measure_tgen.py` 로 재측정해 `tgen_measurement.json` 에 기록하고 원고
  Table 4 의 T_gen 정의를 그에 맞춘다.
- (2차 내부검토 반영) run 1(단계별 타이머 합산, `tgen_measurement_run1_stagewise.json`)의
  "감사 조건 4.6 s"는 원 점수 시간 + 별도 측정 비점수 단계의 **재구성 추정치**이지 실측이 아니며,
  부하 실행 평균 7.0 s 도 "상한"이 아니다. run 2 는 연속 외부 타이머로 ungated 경로 전체와
  게이트 진단(K=4,096 샘플링·점수·NMS·정렬·판정)을 실측한다. Table 4 는 run 2 실측 평균을 쓰고
  재구성 추정치는 민감도 시나리오로만 표기한다.
