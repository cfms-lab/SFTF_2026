...
\title{Fast Prediction of Support-Minimizing Build Orientation for 3D Printing Using a Support Flow Tensor}
\author{InHwan Sul}
\date{}
...
the proposed method was consistently superior to directions based on PCA and Support Tensor eigenvectors.
...
\begin{tikzpicture}[iso, scale=1.2]
  ...
  \begin{scope}
    ...
    \node[font=\bfseries] at (0.8,0.8,-0.75) {(a)};
  \end{scope}
  ...
  \begin{scope}[xshift=7.2cm]
    ...
    \draw[->,line width=1.1pt] (-1.15,0.4,1.0)--++(0,0,1.0) node[above,inner sep=1pt]{$n$};
    \node[font=\bfseries] at (0.9,0.9,-0.75) {(b)};
  \end{scope}
\end{tikzpicture}}
\caption{The two flow terms of the unified tensor. (a) \emph{Face-to-face flow}: when overhang $i$ is supported by an actual face $j$, the receiver normal is
$m_j\cdot n\in(\eta,1)$ and the tensor term is $m_im_j^T$. (b) \emph{Build-plate (virtual ground node)}: when there is no face to support it,
...
We also address adaptive sampling for cases where candidate generation does not include a near-optimal direction (e.g., happy).
...
These figures were rendered from the stored npz grids, the candidate pools (\texttt{Group\_C*\_*\_candidates.csv}), and the verification records (\texttt{Group\_C*\_*.json})
with \texttt{scripts/render\_groupC\_combined.py}.
