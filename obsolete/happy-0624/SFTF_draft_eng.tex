...
\title{Build-Plate-Based Fast Candidate Generation and Slicer Warm-Start for Build Orientation in Additive Manufacturing:\\A Support Flow Tensor Field Approach}
\author{InHwan Sul}
\date{}
...
\begin{abstract}
In additive manufacturing, the support volume depends heavily on the build orientation, so finding a build orientation that minimizes the support structure is important.
The authors' prior GPU-based support structure tomography (GPU-MSST, referred to in this paper as TOMO\_INT3)
yields an accurate support volume through a yaw--pitch global search, but it only makes a single-direction evaluation faster while
\emph{exhaustively} evaluating $129{,}600$ directions, so it incurs a large fixed cost that is independent of shape complexity.
This study does not aim to replace this exhaustive search, but rather proposes a candidate generator that quickly suggests promising directions in front of it,
serving simultaneously as a slicer/TOMO warm-start.
Specifically, it evaluates a low-cost support cost for each candidate direction on the sphere to narrow down a small number of promising orientations,
and feeds these only into the local verification of TOMO\_INT3 or a commercial slicer, thereby reducing the overall search cost.

This study casts support-structure estimation in 3D printing as a \emph{direction-dependent} problem: the per-direction cost
is built around an \emph{asymmetric flow tensor}, the Support Flow Tensor Field $F(n)$, evaluated directly for direction $n$.
Unlike the conventional symmetric tensor (the baseline ``Support Tensor'') that only captures the normal distribution,
it directly reflects, via ray casting, the face-to-face support flow from an overhang face toward the actual receiver face beneath it.
Correlation analysis with the actual support volume shows that most of the predictive power originates from the build-plate support cost and the face-to-face internal visibility term,
while the contribution of higher-order tensor terms such as the nuclear norm was small.
Furthermore, we show that the build-plate support cost is exactly equivalent to the Rayleigh contribution obtained by adding a virtual ground node
(a limiting receiver face whose normal is the build direction) to the flow tensor ($\max|\tilde R-(R+B)|<10^{-12}$),
confirming that the very term that drives the predictive power can itself be expressed as a single tensor contraction
(\emph{ground-node unification}, unified Rayleigh cost $\tilde R=R+B$).
This unified term, by excluding the face-to-face support volume term that acts as a negative signal, consistently shows higher predictive power than the conventional summed objective function.

In correlation analysis, ablation, and Leave-One-Mesh-Out cross-validation on a stored TOMO\_INT3 grid,
the conservative ranking-only evaluation gave an average best-of-3 ratio of $2.37$,
which was consistently superior to directions based on PCA and Support Tensor eigenvectors but still left a gap to the TOMO optimum.
When SFTF is used as intended as a warm-start candidate generator followed by a small local TOMO verification,
the budget curve is substantially stronger: checking the $\pm 10^\circ$ neighborhoods of the top-$20$ SFTF candidates uses only $3.57\%$ of the $1^\circ$ grid on average
and reduces the mean ratio to $1.13$ (maximum $1.58$ over the five benchmark meshes).
A budget-matched control confirms that this gain comes from where SFTF places the verification windows:
uniform and random local-window baselines at the same mean grid budget give mean/max ratios of $1.64/2.79$ and $2.62/5.44$, respectively.
We further introduce the \emph{happy-method}, an automatic hard-case escalation rule triggered by local-verification confidence signals rather than by manual mesh labels.
Starting from top-$10$ local verification, it detects only the happy 50k case in the five stored meshes and expands that mesh to a top-$30$ verification budget,
reducing the maximum ratio from $8.52$ to $1.06$ while using only $2.99\%$ of the full $1^\circ$ grid on average.
On the ratio-valid cached five-group $3^\circ$ audit, the tiered happy-method triggers $7/16$ records and reduces the maximum ratio from $12.44$ to $1.62$ at a mean grid budget of $4.96\%$.
In addition, on a difficulty-controlled five-group shape set (from basic shapes up to large meshes with $\sim$$10^6$ faces, based on a $3^\circ$ exhaustive sweep),
Python SFTF showed a computation-time reduction of $1.4$--$34\times$ relative to TOMO\_INT3,
and C++ SFTF of $10$--$5{,}160\times$.
Expanding the learning/validation set to $103$ usable mesh--critical-angle records from the five-group cache gave a median selected ratio of $1.34$,
with $80/103$ records below a ratio of $2.0$, while the remaining outliers identify cases requiring separate hard-case handling.
We also confirmed that, although TOMO\_CUDA is faster than INT3 on simple shapes, it is not always superior on real parts and large meshes,
showing that the core gain lies in reducing the number of candidates rather than in changing the exhaustive-sweep backend.
Furthermore, by measuring the change in TOMO optimal orientation and support volume across critical angles $\theta_c\in\{0,30,45,60,90\}^\circ$,
we confirmed that the support volume decreases monotonically as $\theta_c$ increases, while the global optimal orientation shifts.
Finally, after coarse spherical sampling we resample at small angles around good candidates,
and present not a single optimum but several mutually separated near-optimal orientation groups (basins) together.
\end{abstract}
...
\begin{tikzpicture}[>=Latex, line join=round, every node/.style={font=\small}]
  ...
  \draw[->, very thick] (-3.2,2.6) -- (-3.2,3.7) node[above] {$n$ (build direction)};
  % build plate
  \fill[pattern=north east lines, pattern color=gray!55] (-3.6,-0.35) rectangle (3.9,0);
  \draw[thick] (-3.6,0) -- (3.9,0);
  \node[below right, gray!55!black] at (-3.6,-0.34) {build plate $z_{\mathrm{plate}}(n)$};
  ...
  \node[below] at (2.2,1.0) {receiver face $j$};
  ...
  \node[above] at (1.5,3.9) {overhang face $i$ \,($O_i=-m_i\cdot n>0$)};
...
\begin{tikzpicture}[>=Latex, line join=round, every node/.style={font=\small}]
  % ---- left: face-face flow ----
  \begin{scope}
    \node at (1.05,4.05) {\textbf{face-to-face flow}};
  ...
  \begin{scope}[xshift=5.4cm]
    ...
    \node[below] at (1.2,0.35) {build plate (virtual node)};
    \draw[->, thick, gray!55!black] (1.1,2.55) -- (1.2,0.8);
  \end{scope}
\end{tikzpicture}
\caption{The two flow terms of the unified tensor. (Left) When overhang $i$ is supported by an actual face $j$, the receiver normal is
$m_j\cdot n\in(\eta,1)$ and the tensor term is $m_im_j^T$. (Right) When there is no face to support it,
...
\section{Current Implementation Pseudocode}
...
\section{Method Analysis: Local Verification, Feature Contribution, Cross-Validation, Comparison with Existing Methods}
...
\subsection{Local TOMO Verification Budget Curve}
...
\subsection{Happy-Method: Automatic Hard-Case Escalation}
...
\footnote{To handle a large-scale batch that sweeps $36$ meshes over $5$ critical angles ($\theta_c\in\{0,30,45,60,90\}^\circ$) with two backends,
...
\subsection{Discussion: Selection of the Ray Budget $K_{ray}$}
